...
export default function Contact() {
  ...
  const sendEmail = (e: any) => {
    ...
    const input1Value = formData.get("name")?.toString() || "";
    const input2Value = formData.get("email")?.toString() || "";
    const input3Value = formData.get("phone")?.toString() || "";
    const input4Value = formData.get("message")?.toString() || "";

    if (input1Value.trim() == "" || input2Value.trim() == "" || input3Value.trim() == "" || input4Value.trim() == "") {
      alert("Fields cannot be empty");
      return;
    }
    emailjs
      .sendForm("service_tkraqkm", "template_lcss637", form.current, {
        publicKey: "WjuLgz9qrcVzH_2Il",
      })
      .then(() => {
        console.log("SUCCESS!");
...

            {
              status === "failed" && (
                <div className="success-msg flex flex-col items-center justify-center py-20 text-center">
                  <div className="w-20 h-20 border border-red-500/50 rounded-full flex items-center justify-center mb-8">
                    <Ban className="text-red-500" size={32} />
                  </div>
                  <h3 className="text-4xl font-bold text-white mb-4 tracking-tighter">Message Failed.</h3>
                  <p className="text-slate-400 text-lg font-light">Try again Later.</p>
                </div>
              )
            }
              ...
              <form onSubmit={sendEmail} ref={form} className="space-y-8">
                <div className="space-y-8">
                  ...
                  <div className="relative">
                    ...
                    />
                    <label htmlFor="phone" className="absolute left-0 -top-3.5 text-sm font-semibold text-slate-500 transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-slate-400 peer-placeholder-shown:top-4 peer-focus:-top-3.5 peer-focus:text-sm peer-focus:text-primary-400 uppercase tracking-widest">Phone Number</label>
                  </div>
                </div>
